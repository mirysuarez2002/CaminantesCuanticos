...
def circuito(pasos):
    q_posicion = math.ceil(math.log2(2*pasos+1))
    num_max_pos = 2**q_posicion
    centro = num_max_pos // 2 

    qc = QuantumCircuit(q_posicion + 1) #Añadimos el qubit del spin

    #Necesito conocer el valor en binario del centro, para hacer más eficiente el programa
    centro_binario = format(centro, f'0{q_posicion}b') #Paso el número a binario y añado los ceros que sean necesarios para completar la cadena de qubits
    #Coloco al caminante en el centro del sistema, variando los qubits necesarios de 0 a 1
    #Invierto la cadena de bit del centro, ya que qiskit los lee invertidos 
    ...
    for _ in range(pasos):
        ...
        for i in range(1, q_posicion):
            qc.x(i)
        qc.x(0) 
    #Devolvemos el circuito correspondiente
    return qc
...
plt.ylabel("Número de Cúbits", fontsize=15)
plt.xlim(0, 11)
